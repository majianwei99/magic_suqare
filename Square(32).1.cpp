...
typedef struct square
{
    ...
    int threshold_max_my[MAXN], threshold_min_my[MAXN];
} SQUARE;
...
void msquare(SQUARE *ps, int no)
{
    ...
    for (i = 1; i <= NN; i++)
    {
        ...
        if (c > N - 2)
        {
            if ((temprsum + ps->threshold_max_my[N - c] < ps->stdsum) || (temprsum + ps->threshold_min_my[N - c] > ps->stdsum))
                continue;
        }
        if (r > N - 2)
        {
            if ((tempcsum + ps->threshold_max_my[N - r] < ps->stdsum) || (tempcsum + ps->threshold_min_my[N - r] > ps->stdsum))
                continue;
        }
